...
def create_table_users():
    """Создаём таблицы USERS (найденные люди)"""
...
def create_table_seen_users():  # references users(vk_id)
    """Создаём таблицу SEEN_USERS (просмотренные люди)"""
    with connection.cursor() as cursor:
        cursor.execute(
...
def insert_data_users(first_name, last_name, vk_id, vk_link):
    """Вставляем данныев таблицу USERS"""
    with connection.cursor() as cursor:
        cursor.execute(
...
def insert_data_seen_users(vk_id, offset):
    """Вставляем данные в таблицу SEEN_USERS"""
    with connection.cursor() as cursor:
        cursor.execute(
...
def select(offset):
    """Получаем выборку из непросмотренных людей"""
    with connection.cursor() as cursor:
        cursor.execute(
...
def drop_users():
    """Удаляем таблицу USERS каскадом"""
    with connection.cursor() as cursor:
        cursor.execute(
...
def drop_seen_users():
    """Удаляем таблицу SEEN_USERS каскадом"""
    with connection.cursor() as cursor:
        cursor.execute(
...
# Окончательный результат функции (creating_database)
